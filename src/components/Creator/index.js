...
const ConstantNodeBlock = {
  NodeBlock: NodeBlock,
};

const ConstantCanLink = {
...
const Creator = () => {
  const [schema, { onChange, addNode, removeNode, connect }] =
    useSchema(initialStateSchema);

  const [availableNodes] = useState(availableNodesData);
  ...
  const getUniqueLinks = (links) => {
    ...
    Object.keys(linksIn).forEach((link) => {
      linksOut[linksIn[link]] = link;
    });

    return Object.keys(linksOut).map((key) => {
      return {
        output: key,
        input: linksOut[key],
      };
  ...
  const deleteNodeFromSchema = (id) => {
    ...
    const nodeToRemove = _schema?.nodes?.find((node) => {
      if (node.id === id) {
        return true;
      }
      return false;
  ...
  const loadInitialSchema = (payload) => {
    let nodes = [];
    let links = [];
  ...
  const updateStateSidebarEditor = ({
    ...
    switch (type) {
      case types.ON_CHANGE_NODE:
        ...
        const index = _nodes.findIndex((node) => {
  ...
  const onChangeWorkFlowTabHandler = ({
    ...
    switch (type) {
      ...
      case types.ON_IMPORT_WORKFLOW:
        ...
        const _updatedSchema = createSchema(_importSchema);
        ...
        _updatedSchema?.nodes.forEach((node) => {
          addNode(node);
        });

        _updatedSchema?.links.forEach((link) => {
